<template>
  <div class="quality"
    ...
    <el-form :inline="true" class="demo-form-inline">
        ...
        <el-form-item>
            <el-button @click="audioQualityPost">查询</el-button>
        </el-form-item>
    </el-form>
    ...
    <div class="biao" v-for="(item, ins) in currentData" :key="ins">
        ...
        <div v-for="(items, index) in item.audioAnalysisListVOList" :key="index">
            <div class="shu" v-if="items.recognitionResult != null">
                ...
                <span v-else-if="items.recognitionResult.flag === false"><a @click="dialogVisible = true, getClick(items.recognitionResult,item)">错误详情</a></span>
                <span v-else>true</span>
                <span v-if="items.subTaskType === '客户身份识别'||items.subTaskType === '销售员身份识别'"></span>
                <span v-else-if="items.similarity === 'NaN'">1</span>
                <span v-else>{{items.similarity}}</span>
                <span v-if="items.subTaskType === '客户身份识别'||items.subTaskType === '销售员身份识别'"></span>
                <span v-else-if="items.whetherContentMatches.flag === false"><a @click="dialogVisible = true, getClicks(items.whetherContentMatches,item)">错误详情</a></span>
    ...
    //   错误详情
        ...
        currentPage(val){
          ...
          this.newdata.sort(function(a,b){
              return Date.parse(b.data) - Date.parse(a.data);//时间正序
          });
          let a = val*10
          for (var i = a-10; i < a; i++) {
            if(this.newdata.length>i){
              this.currentData.push(this.newdata[i])
            }
          }
        },
        chaxun(){
        ...
        // 质检结果请求
        async audioQualityPost(){
          this.newdata = [];
          this.loading=true
          var data = {
            "keyWord": {videoName:this.id,productName:this.name},
            "page": 1,
            "size": 10
          }
          await this.$global.httpVideoPost(this, '/api/v1/audio_analysis/queryQualityTestList',data).then(res => {
                this.data = res.data.audioAnalysisListVOS;
                if(this.data.length!=0){
                  this.q = res.data.currentPage+2
                }
                for(var i = 0; i < this.data.length; i++){
                  this.data[i].videoName = this.data[i].videoName.match(/-(\S*)/)[1];
                }
                this.chaxun()
            })
